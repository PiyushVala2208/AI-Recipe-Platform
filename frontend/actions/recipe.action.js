    ...
    if (searchResponse.ok) {
      ...
      if (searchData.data && searchData.data.length > 0) {
        ...
        const savedRecipeResponse = await fetch(
          `${STRAPI_URL}/api/saved-racipes?filters[user][id][$eq]=${user.id}&filters[recipe][id][$eq]=${searchData.data[0].id}`,
          {
            headers: {
              Authorization: `Bearer ${STRAPI_API_TOKEN}`,
            },
...
export async function saveRecipeToCollection(formData) {
  try {
    ...
    const existingResponse = await fetch(
      `${STRAPI_URL}/api/saved-racipes?filters[user][id][$eq]=${user.id}&filters[recipe][id][$eq]=${recipeId}`,
      {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
    ...
    const saveResponse = await fetch(`${STRAPI_URL}/api/saved-racipes`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${STRAPI_API_TOKEN}`,
...
export async function removeRecipeFromCollection(formData) {
  try {
    ...
    const deleteResponse = await fetch(
      `${STRAPI_URL}/api/saved-racipes/${savedRecipeId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
...
export async function getSavedRecipes() {
  try {
    ...
    const response = await fetch(
      `${STRAPI_URL}/api/saved-racipes?filters[user][id][$eq]=${user.id}&populate[recipe][populate]=*&sort=savedAt:desc`,
      {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
